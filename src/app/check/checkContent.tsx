  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-[#898AC4]">
      <div className="relative p-4 bg-[#A2AADB] rounded-2xl sm:max-w-10/12 max-w-10/12">
        ...
        <div className="grid sm:grid-cols-15 gap-3 sm:gap-6">
          <div className="col-span-6 sm:col-span-8 sm:mt-3">
            ...
            <div className="flex flex-col bg-[#898AC4] rounded-xl text-[#FFF2E0] max-w-1/2 min-w-full p-2">
              <p id="temp" className="text-3xl font-semibold text-center translate-x-1">
                {weather?.current_condition?.[0]?.temp_C ? `${weather.current_condition[0].temp_C}°` : "N/A"}
              </p>
              <p id="locate" className="text-xl font-medium text-center -translate-y-1">
              ...
              <div className="flex gap-2 justify-center text-2xl font-normal text-center">
                ...
                <span className="flex items-center">
                  <svg className="text-[#FFA281]" xmlns="http://www.w3.org/2000/svg" width={16} height={16} viewBox="0 0 24 24">
                    <path fill="currentColor" fillRule="evenodd" d="M11.47 3.47a.75.75 0 0 1 1.06 0l6 6a.75.75 0 1 1-1.06 1.06l-4.72-4.72V20a.75.75 0 0 1-1.5 0V5.81l-4.72 4.72a.75.75 0 1 1-1.06-1.06z" clipRule="evenodd"></path>
                  </svg>
                  <p className="text-sm font-light">
                    {weather?.current_condition?.[0]?.FeelsLikeC ? `${weather.weather[0].maxtempC}°` : "N/A"}
                  </p>
                </span>
                <span className="flex items-center">
                  <svg className="text-[#C0C9EE] rotate-180" xmlns="http://www.w3.org/2000/svg" width={16} height={16} viewBox="0 0 24 24">
                    <path fill="currentColor" fillRule="evenodd" d="M11.47 3.47a.75.75 0 0 1 1.06 0l6 6a.75.75 0 1 1-1.06 1.06l-4.72-4.72V20a.75.75 0 0 1-1.5 0V5.81l-4.72 4.72a.75.75 0 1 1-1.06-1.06z" clipRule="evenodd"></path>
                  </svg>
                  <p className="text-sm font-light">
                    {weather?.current_condition?.[0]?.FeelsLikeC ? `${weather.weather[0].mintempC}°` : "N/A"}
                  </p>
                </span>
                <ToolTip text="Uv Index">
                  <span className="flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width={16} height={16} viewBox="0 0 24 24">
                      <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 3v1.5m5 8.5a5 5 0 0 0-10 0M5.988 6.99l-1.06-1.061M22 13h-1.5m-17 0H2m17.07-7.071l-1.06 1.06M6.5 16v3c0 .943 0 1.414.293 1.707S7.557 21 8.5 21v0c.943 0 1.414 0 1.707-.293s.293-.764.293-1.707v-3m3 0l2 5l2-5" color="currentColor"></path>
                    </svg>
                    <p className="text-sm font-light">
                      {weather?.current_condition[0].uvIndex ? `${weather.current_condition[0].uvIndex}` : "N/A"}
                    </p>
                  </span>
                </ToolTip>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-3"> {/* Card */}
              <Card svg={
        ...
        <button name="Search Another Location" className="sm:hidden text-md xs:text-md sm:text-xs md:text-sm bg-[#898AC4] rounded-md text-[#FFF2E0] p-2 mt-3 w-full outline-2 outline-[#C0C9EE] hover:bg-[#C0C9EE] hover:outline-[#898AC4] hover:text-[#898AC4] active:scale-85 duration-100 cursor-pointer" onClick={() => router.push("/")}>Search Another Location</button>
        <p className="mt-3 text-xs text-[#FFF2E0]">this in celcius format. Data provided by <Link href="https://github.com/chubin/wttr.in">wttr.in</Link></p>
      </div>
    </div>
  );
}